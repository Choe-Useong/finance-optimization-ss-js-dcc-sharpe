...
* 본 접근은 평균-분산 최적화의 고질적인 문제인 **기대수익률 추정오차 증폭(error maximization)**을 완화하는 데 초점을 둡니다.
* 기대수익률은 상태공간(Local-level) 기반으로 시변 추정하고, 횡단면에서는 James–Stein shrinkage를 적용해 추정 분산을 축소합니다.
* 공분산은 DCC-GARCH로 추정하여, 분산·상관의 동학을 분리해 반영합니다.

### 2) 성과표 해석 프레임

* 누적수익률/CAGR은 전략의 절대 성과를, MDD는 tail-risk 구간의 손실 내구성을 보여줍니다.
* Sharpe·Sortino·Calmar를 함께 보면, 수익 대비 변동성/하방위험/낙폭 효율을 다면적으로 평가할 수 있습니다.
* 본 결과에서 Sharpe 전략의 우위는 단순 레버리지 효과보다 **추정 안정화 + 동적 리스크 반영**의 결합 효과로 해석하는 것이 타당합니다.
